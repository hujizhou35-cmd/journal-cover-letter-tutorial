...
Look for at least one evidence-supported pattern:

- a concept the literature routinely conflates;
- an inference widely accepted but insufficiently tested;
- apparently consistent findings that conceal a meaningful contradiction;
- an important layer omitted by previous reviews;
- a pattern hidden by weak classification;
- a conclusion extrapolated beyond the evidence;
- or, for descriptive/scoping work, a map whose absence obstructs the next research decision.

Do not use “no prior review exists” as the sole field problem. Do not manufacture a misreading when the evidence supports only fragmentation or mapping.

## Step 2: Define the synthesis intervention

State what the review does to the literature, for example:

- reorganizes studies under a more informative framework;
- compares evidence across previously separated levels;
- distinguishes concepts that prior work treated as interchangeable;
- exposes a recurring gap or validity failure;
- reconciles conflicting results through a new classification;
- delineates what current evidence supports and does not support.
...
- interpretive rather than procedural;
- contestable enough to matter, but not sensational;
- consequential for a research, policy, education, or practice decision;
- broader than a single result but narrower than an unsupported field-wide verdict.
